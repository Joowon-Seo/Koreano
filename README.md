...
- **Redis 분산 락**: 동시성 제어 및 트랜잭션 직렬화로 재고 충돌 해결
...
프로젝트 초기에 11개의 마이크로서비스로 설계하며 과도한 분리를 시도했습니다. 하지만 팀 규모(백엔드 4명)와 프로젝트 범위(2개월)를 고려했을 때, 더 간소화된 구조가 적합했을 가능성이 높았습니다. 이로 인해 초기 개발 속도가 느려지고, 불필요한 복잡성이 추가된 점이 아쉬웠습니다. 이를 통해 요구사항과 리소스에 맞춘 적정 수준의 설계가 중요하다는 점을 깨달았습니다. 이후 다시 설계하며 **장바구니, 주문, 재고를 하나의 MS로 통합**하는 과정을 거쳤습니다. 이렇게 핵심 도메인을 단일 서비스로 묶자 코드 관리와 배포가 간편해졌고, 팀의 생산성이 향상되었습니다. "YAGNI(You Aren’t Gonna Need It)" 원칙을 실무에 적용하며 과도한 분리 대신 실용성을 우선시하는 법을 배웠습니다.
